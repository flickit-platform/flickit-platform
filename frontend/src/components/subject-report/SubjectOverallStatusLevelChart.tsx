...
const SubjectOverallStatusLevelChart = (props: any) => {
  ...
  const { title, status, confidence_level_value: cl = 1, maturity_level_value: ml } = data;

  return <VerticalLevelChart cl={cl} ml={ml} title={title} status={status} loading={loading} />;
};

...
def add_user_to_space(space_id,current_user, email):
    ...
    if current_user not in space.users.all():
        return ActionResult(success=False , code='currentuser-not-member', message='Only member users of the space can add users to the space.')
    try:
        ...
        return ActionResult(success=True, code='user-joined-space', message='This user has successfully joined this space.')

...
def assessment_core_dsl_update(body, assessment_kit_id, header_authorization):
    ...
    response = requests.put(ASSESSMENT_URL + f'assessment-core/api/assessment-kits/{assessment_kit_id}/update-by-dsl',
                            json=data, headers={'Authorization': header_authorization})
...
def dsl_parser_update(assessment_kit_id, dsl_id, header_authorization):
    dsl_contents = importassessmentkitservice.extract_dsl_contents(dsl_id)
    result = dsl_parser_upload_dsl(dsl_contents)
    if result["status_code"] == status.HTTP_200_OK:
        result = assessment_core_dsl_update(result["body"], assessment_kit_id, header_authorization)
    return result

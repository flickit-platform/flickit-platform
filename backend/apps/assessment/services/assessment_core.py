...
def get_path_info_with_assessment_id(request, assessments_details):
    ...
    if "questionnaireId" in request.query_params:
        if Questionnaire.objects.filter(id=request.query_params["questionnaireId"]).filter(
                kit_version=kit.kit_version_id).exists():
            questionnaire_object = Questionnaire.objects.get(id=request.query_params["questionnaireId"])
            questionnaire = {"id": questionnaire_object.id,
                             "title": questionnaire_object.title
                             }
        else:
            result["Success"] = False
            result["body"] = {"code": "NOT_FOUND", "message": "'questionnaireId' does not exist"}
            result["status_code"] = status.HTTP_400_BAD_REQUEST
            return result

...
def init_quality_attribute_value(result_id):
    ...
    subjects = assessment_result.assessment_project.assessment_kit.assessment_subjects.all()
    atts = []
    maturity_level = maturitylevelservices.extract_maturity_level_by_value(assessment_kit=assessment_result.assessment_project.assessment_kit, value = 0)
    for sub in subjects:
...
def save_qauality_att_value(metric_impacts, assessment_result):
    ...
    for quality_attribute in quality_attributes:
        ...
        maturity_level = maturitylevelservices.extract_maturity_level_by_value(assessment_kit=assessment_result.assessment_project.assessment_kit, value = maturity_level_value)
        try:

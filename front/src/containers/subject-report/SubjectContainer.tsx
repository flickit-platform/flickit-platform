...
const useSubject = () => {
  ...
  const subjectQueryData = useQuery({
    service: (subjectId: string, resultId: string) =>
      service.fetchSubject(subjectId, resultId),
    runOnMount: false,
  });
  ...
  const noStatus =
    !subjectQueryData.data?.status ||
    subjectQueryData.data?.status === "Not Calculated";
...
const NoInsightYetMessage = (props: any) => {
  const { data = {} } = props;

...
export const SpaceMembers = (props: any) => {
  ...
  const spaceData = useQuery({
    service: () => service.fetchSpaceMembers(spaceId),
  });
  const {
    query: addMember,
    loading,
    ...
    service: (id = spaceId, value = user_id_ref.current?.value) =>
      service.addMember(id, value),
    runOnMount: false,
    toastError: (err) => {
      ...
      <Box>
        ...
        <form
          onSubmit={(e) => {
            ...
            } else addMember(spaceId, user_id_ref.current?.value);
          }}
        >
          <TextField
            fullWidth
...
const AddMemberButton = ({ loading }: any) => {
...
const Actions = (props: any) => {
  ...
  const { query: deleteSpaceMember, loading } = useQuery({
    service: () => service.deleteSpaceMember(spaceId, member.id),
    runOnMount: false,
    toastError: true,
  });
